...
This application provides a minimal CORECONF server implementation on top of
the gcoap CoAP stack. See the [CoAP spec][1] for background, and the
...
### Send CORECONF requests to the server

Sending requests to the CORECONF server can be done via

    aiocoap-client coap://[fe80::3885:bff:fef3:4b63%tapbr0]/c/X9 --pretty-print

This requires the python aiocoap module to be installed, for example via

    pip install --user aiocoap
