...
#define HARDWARE_HARDWARE_SID                           HARDWARE_MODULE_SID + 4
#define HARDWARE_HARDWARE_COMPONENT_SID                 HARDWARE_MODULE_SID + 14
#define HARDWARE_HARDWARE_COMPONENT_CLASS_SID           HARDWARE_MODULE_SID + 17
#define HARDWARE_HARDWARE_COMPONENT_NAME_SID            HARDWARE_MODULE_SID + 26
...
static const char *_devname(const saul_reg_t *dev) {
    if (dev->name == NULL) {
        return "(no name)";
    } else {
        return dev->name;
...
static void _fmt_hw_hardware_component_class(coreconf_encoder_t *enc, const coreconf_node_t *node, const saul_reg_t *dev)
{
    (void)dev;
    (void)node;
...
static void _fmt_hw_hardware_component_name(coreconf_encoder_t *enc, const coreconf_node_t *node, const saul_reg_t *dev)
{
    (void)node;
    nanocbor_put_tstr(coreconf_encoder_cbor(enc), _devname(dev));
}
...
static int _fmt_hw_hardware_component_inst(coreconf_encoder_t *enc, const coreconf_node_t *node, const saul_reg_t *dev)
{
    ...
    _fmt_hw_hardware_component_class(enc, node, dev);

    coreconf_cbor_sid(enc, HARDWARE_HARDWARE_COMPONENT_SID,
            HARDWARE_HARDWARE_COMPONENT_NAME_SID);
    _fmt_hw_hardware_component_name(enc, node, dev);
...
static int _fmt_hw_hardware_component(coreconf_encoder_t *enc, const coreconf_node_t *node)
{
    ...
    saul_reg_t *dev = saul_reg;
    while (dev) {
        int res = _fmt_hw_hardware_component_inst(enc, node, dev);
        if (res < 0) {
            return res;
        }
        dev = dev->next;
...
static int _hw_hardware(coreconf_encoder_t *enc, const coreconf_node_t *node)
{
    (void)node;
    nanocbor_fmt_map(coreconf_encoder_cbor(enc), 1);
    coreconf_cbor_sid(enc, HARDWARE_HARDWARE_SID,
            HARDWARE_HARDWARE_COMPONENT_SID);

    return _fmt_hw_hardware_component(enc, node);
}

CORECONF_NODE(HARDWARE_HARDWARE_SID, COAP_GET, _hw_hardware, NULL);
