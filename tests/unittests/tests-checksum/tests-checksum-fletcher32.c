...
static void test_checksum_fletcher32_wrap_around_piecewise(void)
{
    ...
    fletcher32_update(&ctx, (const uint16_t*)wrap_around_data, initial_len / 2);
    fletcher32_update(&ctx, (const uint16_t*)(wrap_around_data + initial_len), (full_len - initial_len) / 2);
